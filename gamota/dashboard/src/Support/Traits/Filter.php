...
trait Filter
{
    ...
    public function scopeBaseFilter($query, $args = [])
    {
        ...
        if (! empty($args['_orderby']) && ! empty($args['_sort'])) {
            $query->orderBy( $this->table.'.'.$args['_orderby'], $args['_sort'] );//edit by Tan : $this->table.'.'.
        }
    ...
    public static function scopeLinkSort($query, $text, $fieldOrder)
    {
        ...
        if( isset( $filter['_sort'] ) ){
            $_sort = ( $filter['_sort'] == 'desc' ? 'asc' : 'desc' );
        }

        // dd($fieldOrder);
        // dd($filter);
        $url = \Request::fullUrlWithQuery(['_orderby' => $fieldOrder, '_sort' => $_sort ]); //edit by Tan
