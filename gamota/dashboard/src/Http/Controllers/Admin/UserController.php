...
class UserController extends AdminController
{
    ...
    public function appplyStatus(Request $request,$status='enable')
    {
        ...
        $arrId=array_diff($arrId, [Auth::id()]); // remove id this user
        $title=trans('cms.success');
        $message=trans('user.destroy-user-success');
        $code=200;
        if($status!='destroy'){
            if($status=='enable'){
                $status=1;
                $message=trans('user.enabel-user-success');
            }else{
                $status=0;
                $message=trans('user.disable-user-success');
            }
            $flag = User::whereIn('id',$arrId)->update(['status'=>$status]);
        }
        else{
            $flag = User::whereIn('id',$arrId)->delete();
        }
        if(!$flag){
            $code=422;
            $title=trans('cms.error');
