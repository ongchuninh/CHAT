@extends('Cms::layouts.default',[
	...
	'breadcrumbs' 			=> [
		'title'	=> [trans('gallery.gallery'), trans('gallery.category.category'), isset($category_id) ? trans('cms.add-new') : trans('cms.edit')],
		'url'	=> [
...
@section('page_title', isset($category_id) ? trans('gallery.category.edit-category') : trans('gallery.category.add-new-category'))
...
@section('content')
	{!! Form::ajax(['url' => isset($category_id) ? route('admin.gallery.category.update', ['id' => $category->id])  : route('admin.gallery.category.store'), 'class' => 'form-horizontal form-bordered form-row-stripped', 'method' => isset($category_id) ? 'PUT' : 'POST']) !!}
		<div class="form-body">
			<div class="form-group">
				<label class="control-label col-sm-3 pull-left">
					@lang('gallery.category.title') <span class="required">*</span>
				</label>
				<div class="col-sm-7">
			...
			<div class="form-group">
				...
				<div class="col-sm-7">
					...
					<label class="checkbox-inline">
						...
						@lang('gallery.category.from-category-title')
					</label>
				</div>
			</div>
			<div class="form-group">
                <label class="control-label col-md-3">
                    @lang('gallery.category.category-parent') <span class="required">*</span>
                </label>
                <div class="col-md-7">
                    @include('Gallery::admin.components.form-select-category', [
                        'categories' => $category->parentAble()->get(),
                        'name' => 'category[parent_id]',
                        'selected' => isset($category_id) ? $category->parent_id : '0',
                    ])
                    <span class="help-block"> @lang('gallery.category.empty-is-root') </span>
                </div>
            </div>
			<div class="form-group">
				<label class="control-label col-sm-3 pull-left">
					@lang('gallery.category.icon')
				</label>
				<div class="col-sm-7">
					<input value="{{ $category->icon }}" name="category[icon]" type="text" placeholder="" class="form-control" />
					<span class="help-block"> @lang('gallery.category.use-fa') </span>
				</div>
			</div>
			<div class="form-group">
                <label class="control-label col-md-3">
