    ...
    $.fn.handleTableFunction = function (options) {
        ...
        return this.each(function () {
            ...
            $(object).on('click', '.btn-apply', function (e) {
                ...
                var action = $('.multiple-function option:selected').val();
                console.log(action);


                if (!action) {
                    return toastr.warning('Vui lòng chọn hành động cần thực hiện!');
                }
                var current_status = window.location.search.indexOf('status=disable') > 0 ? 'disable' : 'enable';
                if (action.split('/').reverse()[0] == current_status) {
                    return toastr.warning('Hiện đang xem dữ liệu ở trạng thái này!');
                }
                var option = $('.multiple-function>option[value="' + action + '"]');
                ...
                if ($('tbody input[type="checkbox"]:checked').length < 1) {
                    return toastr.warning('Vui lòng chọn dữ liệu cần thay đổi!');
                }
                $.each($('tbody input[type="checkbox"]:checked'), function (i, item) {
                    id[i] = $(item).val();
                });

                if (method == 'delete' || method == 'DELETE' || option.attr('data-message')) {
